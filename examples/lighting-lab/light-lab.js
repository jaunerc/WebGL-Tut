...
let scene = {
    ...
    lookAtCenter: [0, 0, 0],
    lookAtUp: [0, 1, 0],
    lightPosition: [0, 0, -25], // Position of the light source
...
function prepareScene() {
    setUpProjectionMat();
    let cube1 = new TexturedCube(gl, [0, 10, 30], 8);
    let cube2 = new SolidCube(gl, [0, -10, 30], 8);
    let cube3 = new SolidCube(gl, [10, 0, 30], 8);
    let cube4 = new SolidCube(gl, [-10, 0, 30], 8);
    let cube5 = new SolidCube(gl, [0, 0, 40], 8);

    let lightPosNormalCoordinate = [
        scene.lightPosition[0] * -1,
        scene.lightPosition[1] * -1,
        scene.lightPosition[2] * -1
    ];
    let lightSource = new LightSource(gl, lightPosNormalCoordinate, 1);

    scene.cubes.push(cube1);
    scene.cubes.push(cube2);
    scene.cubes.push(cube3);
    scene.cubes.push(cube4);
    scene.cubes.push(cube5);
    scene.cubes.push(lightSource);
}
